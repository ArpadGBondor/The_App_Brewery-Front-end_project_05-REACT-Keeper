const defaultNotes = [
  {
    title: 'Delegation',
    content: 'Q. How many programmers does it take to change a light bulb? A. None – It’s a hardware problem',
  },
  {
    title: 'Loops',
    content:
      'How to keep a programmer in the shower forever. Show him the shampoo bottle instructions: Lather. Rinse. Repeat.',
  },
  {
    title: 'Arrays',
    content: "Q. Why did the programmer quit his job? A. Because he didn't get arrays.",
  },
  {
    title: 'Hardware vs. Software',
    content:
      "What's the difference between hardware and software? You can hit your hardware with a hammer, but you can only curse at your software.",
  },
];

const notesObject = {
  load: () => {
    let notesArray = JSON.parse(localStorage.getItem('keeper-app-notes'));
    return notesArray ? notesArray : defaultNotes;
  },
  save: (notes) => {
    localStorage.setItem('keeper-app-notes', JSON.stringify(notes));
  },
};

export default notesObject;
